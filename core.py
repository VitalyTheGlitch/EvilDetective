...
class BaseWindow:
    def __init__(self, root=None, title='Evil Detective', **kwargs):
        ...
        logo_ = os.path.join(config.CODEPATH, 'assets', 'logo.png')
    ...
    def set_icon(self):
        if 'win32' in sys.platform:
            icon_ = os.path.join(config.CODEPATH, 'assets', 'logo.ico')

            self.root.iconbitmap(default=icon_)

        elif 'linux' in sys.platform:
            img_ = tk.Image('photo', file=os.path.join(config.CODEPATH, 'assets', 'logo.png'))
    ...
    def get_monospace_font():
        ...
        return fonts.get(sys.platform, font.Font(size=9, family='Monospace'))

...
class PasswordCrack:
    ...
    @staticmethod
    def make_pin(pin, length):
        return ''.join(pin).zfill(length).encode()

    def _sam_algo(self, pin, times=1024):
    ...
    def get_total_combos(self):
        return sum(len(self.alpha_range) ** i for i in range(self.min_len, self.max_len + 1))

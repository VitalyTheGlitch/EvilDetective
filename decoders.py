...
class LocksettingsDecoder(AndroidDecoder):
    ...
    def main(self):
        ...
        self.DICT = collections.ChainMap(*[self.name_val(d) for d in self.sql_table_as_dict(table)])
...
def parse_lockscreen_wal(file_path):
    with open(file_path, 'rb') as R:
        re_res = re.findall(rb'_salt(\-?\d+)', R.read(), re.DOTALL)

...
def sqlite_error_retry(method):
    ...
    def decorator(*args, **kwargs):
        try:
            ...
            if 'UTF-8' in str(err):
                return method(*args, **kwargs, cursor_kw={'text_factory': AndroidDecoder.decode_safe})
...
class AndroidDecoder:
    ...
    def get_namespace(name):
        ...
        return namespaces.get(name, name)
    ...
    def check_sqlite_magic(self):
        if os.path.isfile(self.input_file):
            with open(self.input_file, 'rb') as R:
                if R.read(len(config.SQLITE_MAGIC)) == config.SQLITE_MAGIC:
                    return True
        ...
        raise DecoderError(f'Target file `{os.path.basename(self.input_file)}` is not an SQLite database or it is encrypted!')
    ...
    def http_status(status):
        ...
        return codes.get(status, f'Code({status})')
